...
export function InQueueMessagingHelper(flex, manager) {
  ...
  CallbackChannel.templates.TaskListItem.firstLine = (task) =>
    `${task.queueName}: ${task.attributes.name}`;
  CallbackChannel.templates.TaskCanvasHeader.title = (task) =>
    `${task.queueName}: ${task.attributes.name}`;
  CallbackChannel.templates.IncomingTaskCanvas.firstLine = (task) =>
    task.queueName;
  // Lead Channel Icon
  CallbackChannel.icons.active = (
    <PhoneCallbackIcon key='active-callback-icon' />
  );
  ...
  flex.TaskInfoPanel.Content.replace(
    <InQueueCallbackComponent key='demo-component' manager={manager} />,
    {
      sortOrder: -1,
  ...
  // Basic Voicemail Channel Settings
  VoiceMailChannel.templates.TaskListItem.firstLine = (task) =>
    `${task.queueName}: ${task.attributes.name}`;
  VoiceMailChannel.templates.TaskCanvasHeader.title = (task) =>
    `${task.queueName}: ${task.attributes.name}`;
  VoiceMailChannel.templates.IncomingTaskCanvas.firstLine = (task) =>
    task.queueName;
  // Lead Channel Icon
  ...
  flex.TaskInfoPanel.Content.replace(
    <InQueueVoicemailComponent key='demo-component' manager={manager} />,
    {
      sortOrder: -1,
